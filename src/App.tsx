...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex font-sans selection:bg-indigo-500/30">
      <aside className="w-64 bg-zinc-900 border-r border-zinc-800 p-6 flex flex-col fixed h-full md:relative z-40">
        ...
        <nav className="flex-grow space-y-1">
          ...
          <MenuBtn active={currentView === 'gigs'} onClick={() => setCurrentView('gigs')} icon={<Briefcase size={20}/>} label="Shows / Agenda" />
          <MenuBtn active={currentView === 'crm'} onClick={() => setCurrentView('crm')} icon={<Users size={20}/>} label="CRM / Leads" />
          <MenuBtn active={currentView === 'finance'} onClick={() => setCurrentView('finance')} icon={<DollarSign size={20}/>} label="Financeiro" />
          {isAdmin && <MenuBtn active={currentView === 'userManagement'} onClick={() => setCurrentView('userManagement')} icon={<UserCheck size={20}/>} label="Painel Admin" />}
        </nav>
        ...
        {currentView === 'dashboard' && (
          <div className="space-y-8 animate-in fade-in duration-500">
            <header className="flex justify-between items-end border-b border-indigo-500/30 pb-4">
              <h2 className="text-4xl font-black italic uppercase tracking-tighter">Dashboard BI</h2>
              <div className="text-right">
                <p className="text-[10px] text-zinc-500 font-black uppercase">Saldo Total</p>
                <p className={`text-3xl font-black ${biData.saldoGeral >= 0 ? 'text-teal-400' : 'text-red-500'}`}>R$ {biData.saldoGeral.toLocaleString('pt-BR')}</p>
              </div>
            </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-bottom-2 duration-300">
            <div className="space-y-6">
              <div className="flex justify-between items-center border-b border-indigo-500/30 pb-4">
                ...
                <button onClick={() => setModalOpen({ type: 'gigs', data: null })} className="bg-indigo-600 px-6 py-3 rounded-2xl flex items-center hover:bg-indigo-700 transition-all font-bold">
                  <PlusCircle size={20} className="mr-2"/> Novo Show
                </button>
              </div>
              <div className="bg-zinc-900/50 rounded-3xl border border-zinc-800 overflow-hidden shadow-2xl">
            ...
            <div className="bg-zinc-900 p-8 rounded-[40px] border border-zinc-800 shadow-2xl h-fit">
              ...
              <div className="grid grid-cols-7 gap-2">
                ...
                {Array.from({length: 31}, (_, i) => {
                  ...
                  return (
                    <div key={i} className={`h-10 flex items-center justify-center rounded-xl text-xs font-bold transition-all ${temShow ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30 scale-110' : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700'}`}>
                      {i + 1}
                    </div>
                  );
                })}
              </div>
              <p className="mt-6 text-[10px] text-zinc-500 italic">* Marcadores indicam shows confirmados no mês atual.</p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-6 animate-in slide-in-from-bottom-2 duration-300">
            <div className="flex justify-between items-center border-b border-indigo-500/30 pb-4">
              ...
              <div className="flex gap-2">
                {currentView === 'finance' && (
                  <button onClick={() => setModalOpen({type: 'categories', data: null})} className="bg-zinc-800 px-4 py-2 rounded-xl flex items-center hover:bg-zinc-700 text-xs font-bold border border-zinc-700 transition-all">
                    <Settings size={16} className="mr-2"/> Categorias
                  </button>
                )}
                <button onClick={() => setModalOpen({ type: currentView, data: null })} className="bg-indigo-600 px-6 py-3 rounded-2xl flex items-center hover:bg-indigo-700 transition-all font-bold">
      ...
      {modalOpen.type && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-4 z-50 backdrop-blur-md">
          <div className={`bg-zinc-900 border border-zinc-800 p-8 rounded-[40px] w-full shadow-2xl relative ${modalOpen.type.startsWith('detail_') ? 'max-w-2xl' : 'max-w-md'}`}>
            ...
            <h3 className="text-2xl font-black mb-8 text-indigo-400 uppercase italic">
            ...
            {modalOpen.type === 'categories' && (
              <form onSubmit={(e) => {
                ...
                <textarea name="categories" defaultValue={userCategories.join(', ')} className="w-full p-5 bg-zinc-800 rounded-3xl border border-zinc-700 outline-none h-32 focus:border-indigo-500" required />
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                {biData.clientes.map(([nome, valor]: any) => (
                  <div key={nome} className="flex justify-between p-4 bg-zinc-800/50 rounded-2xl border border-zinc-700">
                    ...
                    <span className="text-teal-400 font-black">R$ {valor.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
                {biData.despesas.map(([cat, valor]: any) => (
                  <div key={cat} className="flex justify-between p-4 bg-zinc-800/50 rounded-2xl border border-zinc-700">
                    ...
                    <span className="text-red-400 font-black">R$ {valor.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            {!modalOpen.type.startsWith('detail_') && modalOpen.type !== 'categories' && (
              <form onSubmit={(e) => {
                e.preventDefault();
                salvarRegistro(modalOpen.type!, Object.fromEntries(new FormData(e.currentTarget)));
              }} className="space-y-4">
                {modalOpen.type === 'crm' && (
                  <>
                    <input name="name" defaultValue={modalOpen.data?.name} placeholder="Nome do Lead" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <input name="venue" defaultValue={modalOpen.data?.venue} placeholder="Local Provável" className="w-full p-4 bg-zinc-800 rounded-2xl" />
                    <input name="date" type="date" defaultValue={modalOpen.data?.date || '2026-04-13'} className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <select name="status" defaultValue={modalOpen.data?.status || 'novo'} className="w-full p-4 bg-zinc-800 rounded-2xl uppercase font-black text-xs">
                ...
                {modalOpen.type === 'gigs' && (
                  <>
                    <input name="venue" defaultValue={modalOpen.data?.venue} placeholder="Casa de Show" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <input name="date" type="date" defaultValue={modalOpen.data?.date || '2026-04-13'} className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <input name="fee" type="number" defaultValue={modalOpen.data?.fee} placeholder="Cachê R$" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <label className="flex items-center space-x-3 p-4 bg-zinc-800 rounded-2xl cursor-pointer">
                      ...
                      <span className="text-sm font-bold uppercase italic">Cachê Recebido? (Lança em Finanças)</span>
                    </label>
                  </>
                )}
                {modalOpen.type === 'finance' && (
                  <>
                    <input name="description" defaultValue={modalOpen.data?.description} placeholder="Descrição" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <input name="amount" type="number" defaultValue={modalOpen.data?.amount} placeholder="Valor R$" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <select name="category" className="w-full p-4 bg-zinc-800 rounded-2xl uppercase font-black text-xs">
                      {userCategories.map(cat => <option key={cat} value={cat}>{cat.toUpperCase()}</option>)}
                    </select>
                    <select name="type" defaultValue={modalOpen.data?.type || 'entrada'} className="w-full p-4 bg-zinc-800 rounded-2xl uppercase font-black text-xs">
                ...
                {modalOpen.type === 'userManagement' && (
                  <>
                    <input name="name" placeholder="Nome do Músico" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <input name="loginName" placeholder="Usuário" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <input name="password" type="password" placeholder="Senha" className="w-full p-4 bg-zinc-800 rounded-2xl" required />
                    <select name="role" className="w-full p-4 bg-zinc-800 rounded-2xl uppercase font-black text-xs"><option value="usuario">MÚSICO</option><option value="admin">ADMIN</option></select>
                  </>
                )}
                <button type="submit" className="w-full p-5 bg-indigo-600 rounded-[30px] font-black uppercase tracking-widest hover:scale-105 transition-all">Salvar Registro</button>
              </form>
            )}
          </div>
        </div>
...
const InteractiveCard = ({ title, value, icon, color, onClick, isHistory }: any) => (
  <button onClick={onClick} className={`text-left p-8 bg-zinc-900 rounded-[32px] border-l-8 ${color} shadow-2xl hover:scale-[1.02] transition-all group relative overflow-hidden`}>
...
const DataRow = ({ title, sub, val, isPositive, onEdit, onDelete, hideEdit }: any) => (
  <tr className="border-b border-zinc-800 hover:bg-zinc-800/30 transition-colors">
    ...
    <td className={`p-5 text-right font-black italic ${isPositive ? 'text-teal-400' : 'text-zinc-400'}`}>{val}</td>
...
const LoginPage = ({ onLogin }: any) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black p-4">
      <div className="w-full max-w-sm bg-zinc-900 p-12 rounded-[50px] border border-zinc-800 shadow-2xl">
        <h2 className="text-4xl font-black text-center text-indigo-500 italic mb-10 tracking-tighter uppercase">Musicianos</h2>
        <div className="space-y-4 text-center">
          <input placeholder="USUÁRIO" className="w-full p-5 bg-zinc-800 rounded-3xl border border-zinc-700 focus:border-indigo-500 outline-none font-bold text-center" onChange={e => setU(e.target.value)} />
          <input type="password" placeholder="SENHA" className="w-full p-5 bg-zinc-800 rounded-3xl border border-zinc-700 focus:border-indigo-500 outline-none font-bold text-center" onChange={e => setP(e.target.value)} />
